...
class MenuScoreUI extends ui.components.menu.MenuScoreUI {
    ...
    protected listenBtn() {
        this._btn_close.on(Laya.Event.CLICK, this, () => {
            if (MenuScoreUI.TAG_LIST == this._currentTag) {
                this.visible = false
            }
            else this.currentTag = MenuScoreUI.TAG_LIST
        })
    ...
    protected playersArr = []
    public deniedScroll(view, lists: Array<Laya.List>) {
    ...
    public onViewTouch(e: Laya.Event, list: Laya.List) {
        if (list) {
            if (e.type == "mousedown") {
    ...
    private updateListDetail(cell: MenuHistoryDetailRenderer, index: number): void {
        this.deniedScroll(cell._video, [this._list, this._listDetail, this._players])
        cell.playersArr = this.playersArr;
        cell.updata();
    }
    ...
    private onSelectDetail(e: Laya.Event, index): void {
        if (e.type == Laya.Event.CLICK) {
            ...
            if (e.target.name == "video") {
                ...
                gtype = (e.currentTarget as MenuHistoryDetailRenderer).dataSource.gtype
            }
            LoadingUI.instance.show(LoadingUI.TYPE_NO_BG_DELAY);
    ...
    private onGetHistoryList(response: any): void {
        if (response.code == 0) {
            ...
            for (var i: number = 0; i < response.data.length; i++) {
                ...
                if (response.data[i].gmode >= 100) flag = false;
    ...
    private showDetail(gid: number, gtype): void {
        ...
        let space = "     "
        let len = this._detailTotal[gid].length
        this.playersArr = []
        for (var k in this._detailTotal[gid]) {
            ...
            this.playersArr.push(Utils.getFitNickName(v.n, 10))
        }
        
        TextRender.WID = 860 / 6
       // TextRender.width = 860 / len
        this._players.itemRender = TextRender
        this._players.array = this.playersArr
        this._players.renderHandler = new Laya.Handler(this, (cell: TextRender, idx: number) => {
            cell.updata(idx)
            //cell.setWidth(860 / len)
            //this._players.refresh()
        })
        
         //this._listDetail._players = this._players
...
class TextRender extends Laya.View {

    private _txt: Laya.Label
    public static WID: number
    constructor() {
        super()
        this.init()
    }
