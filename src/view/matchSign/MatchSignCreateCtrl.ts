	...
	export class MatchSignCreateCtrl extends BaseCtrl {
        ...
        */
		...
		private checkTab(): void {
			if (this._data.length > 1) {
				...
				for (let k in this._data) {
					let name =GameDef.GAME_NAME[parseInt(this._data[k])  - 1]
        			let createInfo = GameConfig.getCreateInfoByGameName(name)
					let btn = new component.BaseButton("createTable/gameTab.png", createInfo.ch_name)
					btn.stateNum = 3
					btn.labelSize = 28
		...
		private create(): void {
			...
			var desc = "第一名10元红包"
			var matchRule: string = JSON.stringify({ matchmode, title, cost, playercnt, starttime, desc });
