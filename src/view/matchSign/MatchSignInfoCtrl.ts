	...
	export class MatchSignInfoCtrl extends BaseCtrl {
		...
		public beforeShow(): void {
			if (!this._ui) {
				...
				this._ui._rewardpanel.itemRender = MatchSignRewardRenderer;
		...
		*/
		...
		/***渲染单元格时的回调方法***/
		protected updateRewardList(cell: MatchSignRewardRenderer, index: number): void {
			cell.updata();
		}
		...
		private onTouch(e: Laya.Event): void {
			switch (e.currentTarget) {
				...
				case this._ui._tab1:
					...
					MatchSignData.getReward(this._match.sys,function(data){
						console.info(data)
						let reward = []
						for(var k in data){
							let v = data[k]
