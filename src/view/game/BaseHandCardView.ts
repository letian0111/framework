class BaseHandCardView extends Laya.View {
    ...
    protected _cardView;
    public CARD_WIDTH = [-89, 0, 32, 0]
    public CARD_HEIGHT = [0, 29*0.85, 0, -29*0.85]
    public CARD_WIDTH_OFFSET = [-100, 0, 50, 0]
    public CARD_HEIGHT_OFFSET = [0, 50*0.85, 0, -50*0.85]
    public TOUCH_CARD_WIDTH = 90
    public TOUCH_CARD_HEIGHT = 129
    public UPDELAY = 15
    ...
    protected onClickCard(card): void {
    ...
    public showAllCards(cards, ani?) {
        if (ani) {
            this.dealCardsAni(cards)
            return
        }
    ...
    public dealCardsAni(cards) {
        let delay = 0
        for (let k = this.cardsViews.length-8; k >=0 ; k--) {
            let view = this.cardsViews[k];
            let value = this.cardsViews.length-1-k
            value = Math.max(value-7, 0)
            let duration = value*value * 5  
            let height = value*value*value * 1.5
            let y = view.y
            view.y = y - height
            log(view.y)
            Laya.Tween.to(view,{y:y},duration,null,null,delay)
            delay = delay + value * 3     
        }
    ...
    public sortCardsViews() {
        ...
        for (var i = 0; i < length - 1; i++) {
            for (var j = i + 1; j < length; j++) {
                if (BaseGameData.SHIFTER_NUM.indexOf(this.cardsViews[i].card % 100) > -1 || BaseGameData.SHIFTER_NUM.indexOf(this.cardsViews[j].card % 100) > -1) {
                    if (BaseGameData.SHIFTER_NUM.indexOf(this.cardsViews[i].card % 100) > -1 && BaseGameData.SHIFTER_NUM.indexOf(this.cardsViews[j].card % 100) == -1) {
