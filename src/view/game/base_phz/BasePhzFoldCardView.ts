class BasePhzFoldCardView extends Laya.View{
    ...
    private init(seatid,dir){
        ...
        this.cardsViews = []
        this.initData()
        let mode = this.getCardModel()
        this.foldView = new mode()
    ...
    private getCardModel(){
        return ui.daye_phz.FoldCardUI;
    }

    public addFoldCardOne(index, cardid){
        if (this.checkLeft()){
            index = this.MAX_NUM - index
        }
        ...
        view.visible = true
        if (cardid > 0) {
            view._bg.skin = this.RES_PATH + "1/other_" +(cardid%100)+".png"
    ...
    public addFoldCard(cardid) {
        ...
        this.addFoldCardOne(index, cardid)
    }
