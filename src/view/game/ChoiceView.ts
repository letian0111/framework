...
class ChoiceView extends Laya.Sprite{
    protected POS = [[0],[-1,1],[-1,0,1]]
    protected OFFSET = [[0],[40,80],[-150,0,150]]
...
public init(cards,choiceType,optcallback,cancelback){
    ...
    if (BaseGameData.gameType == GameDef.GAME_TYPE.FUYANG_MJ) {
        if (choiceType == GameDef.OptType.MJ_MNGANG || choiceType == GameDef.OptType.MJ_ANGANG) {
            groupCount = 4
            width = -220
            offset = -220
        }
        else if (choiceType == GameDef.OptType.MJ_PENG) {
            groupCount = 3
            width = -170
            offset = -170
        }
