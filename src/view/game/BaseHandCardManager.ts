...
public clearArror(seatid){
	...
	public showDisCardAni(seatid) {
		if (!seatid) {
			return
		}
		if(this.handCardsViews[seatid]){
			this.handCardsViews[seatid].showDisCardAni()
		}
	}

    public removeHandAni(){
        let seatid = BaseGameData.selfSeatid
        this.handCardsViews[seatid].removeDisCardAni()
    }
}