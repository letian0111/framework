...
module poker {
	export class CardUI extends Laya.View {
		...
		public static mouseDownStartCardIndex: number;
		public static mouseDownEndCardIndex: number;
		...
		public set card(value: number) {
			...
			if(BaseGameData.gameType == GameDef.GAME_TYPE.SHUANGKOU || BaseGameData.gameType == GameDef.GAME_TYPE.DABAN ||
			   BaseGameData.gameType == GameDef.GAME_TYPE.DAYE_DB || BaseGameData.gameType == GameDef.GAME_TYPE.DABAN_3P || 
			   BaseGameData.gameType == GameDef.GAME_TYPE.GUANPAI){
				baseUrl = "pokerCardBigL/pokerCardBig_";
		    ...
		    else if (BaseGameData.gameType == GameDef.GAME_TYPE.DOUDIZHU)
				this._imgKengZhu.source = Laya.Loader.getRes(baseUrl + "Doudizhu" + ".png");
			...
			if (value) {
				...
				switch (BaseGameData.gameType) {
					...
					case GameDef.GAME_TYPE.GUANPAI:
						this.setGuanPai(num, numUrl,baseUrl,flower);
						break;
					case GameDef.GAME_TYPE.SHUANGKOU:
						this.setShuangkou(num, numUrl,baseUrl,flower);
						break;
					case GameDef.GAME_TYPE.DABAN:
						this.setShuangkou(num, numUrl,baseUrl,flower);
						break;
					case GameDef.GAME_TYPE.DABAN_3P:
						this.setShuangkou(num, numUrl,baseUrl,flower);
						break;
					case GameDef.GAME_TYPE.DAYE_DB:
						this.setShuangkou(num, numUrl,baseUrl,flower);
						break;
				    case GameDef.GAME_TYPE.DOUDIZHU:
						this.setDoudizhu(num, numUrl,baseUrl);
						break;
					case GameDef.GAME_TYPE.BULL:
						this.setBull(num, numUrl);
						break;
				}
			}
			else {
		...
		private setGuanPai(num: number, numUrl: string,baseUrl:string,flower:number): void {
			this.visible = true
			var imgNum: number = num;
			if (num >= 11 && num <= 13) {
                this._imgFlowerBig.source = Laya.Loader.getRes(baseUrl + "flower_big_" + flower + "_" + num + ".png");
            }
			if (num == 14)
				imgNum = 1;
			else if (num == 16)
		...
		private setShuangkou(num: number, numUrl: string,baseUrl:string,flower:number): void {
			var imgNum: number = num;
			if (num >= 11 && num <= 13) {
                this._imgFlowerBig.source = Laya.Loader.getRes(baseUrl + "flower_big_" + flower + "_" + num + ".png");
            }
			if (num == 14)
				...
				if (this._imgFlowerSmall)
					this._imgFlowerSmall.visible = false;
				this._imgFlowerBig.source = Laya.Loader.getRes(baseUrl + "flower_small_" + 4 + ".png");
			}
			else if (num ==17){
				imgNum = 14;
				...
				if (this._imgFlowerSmall)
					this._imgFlowerSmall.visible = false;
				this._imgFlowerBig.source = Laya.Loader.getRes(baseUrl + "flower_big_" + 4 + ".png");
			}
			this._imgNumber.source = Laya.Loader.getRes(numUrl + imgNum + ".png");
		}

		private setDoudizhu(num: number, numUrl: string, baseUrl:string): void {
			var imgNum: number = num;
			if (num == 16) {
				imgNum = 2;
			}
		...
		private setBull(num: number, numUrl: string): void {
			this._imgNumber.source = Laya.Loader.getRes(numUrl + num + ".png");
		}
		...
		public move(data: any, isEndPosi: boolean = false, needRemoveTouch: boolean = true, needTween: boolean = true, duration: number = 100, callback: any = null, thisObj: any = null, args: any = null, delay: number = 0): void {
			...
			// if(BaseGameData.gameType==GameDef.GAME_TYPE.SHUANGKOU){
			...
			switch (e.type) {
				case Laya.Event.MOUSE_OVER:
					if (CardUI.isMouseDown) {
						...
						//滑动终点的索引
						CardUI.mouseDownEndCardIndex = this._index
				...
				case Laya.Event.MOUSE_DOWN:
					...
					//滑动起点的索引
					CardUI.mouseDownStartCardIndex = this._index
		...
		public play() {
			...
			this._imgFlowerBig.visible = false;
			this._imgFlowerSmall.visible = false;
			let fanpai = new ui.bazhang.fanpaiUI()
