...
class PlaySheet {
    ...
    public init(sheetData) {
        ...
        this.netmsgs.forEach(element => {
            if(element.name == "game.GameStartNtf"){
                if( element.msg.players)
                 BaseGameData.maxPlayer = element.msg.players.length;
            }
        });
        // if(this.netmsgs[1].msg.players){

        //     BaseGameData.maxPlayer = this.netmsgs[1].msg.players.length;

        // }else if(this.netmsgs[0].msg.players)
        // {
        //      BaseGameData.maxPlayer = this.netmsgs[0].msg.players.length;
        // }
        // else
        // {   
        //     log("获取不到最大人数 ，录像报错")
