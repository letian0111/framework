...
* 扑克游戏基础类;
*/
class BasePokerPage extends MatchPage {
    constructor() {
        super();
    ...
    protected _moveCardTime: number = 100;
    protected _myHandCardPosiY: number = 546;
    protected _myFoldCardPosiY: number = 386;
    protected _myFoldCardUpPosiY: number = 347;
    ...
    protected _topFoldCards: Array<poker.CardSmallUI>;
    protected _publicCards: Array<poker.CardSmallUI>;
    protected _sendCards: Array<poker.CardSmallUI>;
    protected _leftShowCards: Array<poker.CardSmallUI>;
    protected _rightShowCards: Array<poker.CardSmallUI>;
    protected _topShowCards: Array<poker.CardSmallUI>;
    protected _friendShowCards: Array<poker.CardBigUI>;
    ...
    protected removeAllCards(): void {
        ...
        this.removeTopFoldCards();
        this.removeLeftShownCards();
        this.removeRightShownCards();
        this.removeTopShownCards();
        this.removePublicCards();
        this.removeSendCards();
        this.removeMyFriendShownCards();
    }

    protected removeSelfHandCards(): void {
    ...
    protected removeTopFoldCards(): void {
        if (!this._topFoldCards)
            this._topFoldCards = [];
        else {
            while (this._topFoldCards.length) {
                var card: poker.CardUI = this._topFoldCards.pop();
                card.removeSelf();
    ...
    protected layoutMyHandCards(needTween: boolean, isSendCards: boolean = false): void {
        // return;
        if (this._selfHandCards.length <= 0) {
            return
        }
    ...
    public autoDisconnect(cards: Array<number>): void {
    ...
    protected layoutMyFoldCards(needTween: boolean, cards: Array<number> = null, noAni: boolean = false): void {
    ...
    protected useCards(seatid: number, cards: Array<number>, isReconnect: boolean, noAni: boolean = false, isShowCard: boolean = false): void {
        ...
        else if (BaseGameData.maxPlayer == 4) {
            if (idx == 1)
                this.useVisiterCards(cards, seatid, isReconnect, noAni);
            else if (idx == 2)
                this.useRightCards(cards, seatid, isReconnect, noAni);
            else if (idx == 3)
                this.useTopCards(cards, seatid, isReconnect, noAni);
            else if (idx == 4)
                this.useLeftCards(cards, seatid, isReconnect, noAni);
        }
    }
    protected useTopCards(cards: Array<number>, seatid: number, isReconnect: boolean, noAni: boolean = false): void {
        this.removeTopFoldCards();
        var card: poker.CardSmallUI;
        for (var i: number = 0; i < cards.length; i++) {
            card = poker.CardSmallUI.borrowCard() as poker.CardSmallUI;
            ...
            if (i == (cards.length - 1))
                ...
                this.setKengZhu(card, -1);
            card.card = cards[i];
            this._topFoldCards.push(card);
            this._pokerCon._boxCards.addChild(card);
        }
        this.layoutTopCards(true, cards, isReconnect, noAni);
    }
    protected layoutTopHandCards(needTween: boolean, isSendCards: boolean = false): void {
        // return;
        var dis: number = (AppControl.getInstance().stage.width - 280) / this._selfHandCards.length;
        if (dis > 50)
            dis = 50;
        ...
        for (var i: number = 0; i < this._selfHandCards.length; i++) {
            ...
            if (isSendCards)
                delay = i * 30;
            this._selfHandCards[i].move({ x: pX, y: AppControl.getInstance().stage.height - 640 + 20, visible: true }, true, false, needTween, time, null, null, null, delay);
            if (i == (this._selfHandCards.length - 1))
                this.setKengZhu(this._selfHandCards[i], BaseGameData.selfSeatid ? BaseGameData.selfSeatid : 1);
            else
                this.setKengZhu(this._selfHandCards[i], -1);
    ...
    protected layoutTopCards(needTween: boolean, cards: Array<number> = null, isReconnect: boolean = false, noAni: boolean = false): void {
      		if (this._topFoldCards.length) {
			var card: poker.CardSmallUI = this._topFoldCards[0];
			var dis: number;
			var pX: number;
			var pY: number;
			var scale: number;
			if (this._topFoldCards.length > 14) {
				dis = 20;
				scale = 0.4;
			}
			else {
			...
			var centerX: number = this._pokerCon.width * 0.5;
			var half: number = Math.ceil(this._topFoldCards.length * 0.5) - 1;
			if (half > 13)
				half = 13;
			//Math.min(1280,
			//+this._topFoldCards.length*card.width * scale * 0.5) 
			//- 183 - card.width * scale * 0.5;
			
			let opseatid = (BaseGameData.selfSeatid + 1) % 4 + 1

			if(BaseGameData.selfSeatid == 0){
				opseatid = 3
			}
			var upY: number = PlayerManager.instance._players[opseatid].y + card.height * scale * 0.5 + 30 + 80 + 20;//+20
			var downY: number = PlayerManager.instance._players[opseatid].y + card.height * scale * 0.5 + 40 + 30 + 80 + 20;//+76
			var middleY: number = PlayerManager.instance._players[opseatid].y + card.height * scale * 0.5 + 25 + 30 + 80 + 20;


			if (this._topShowCards.length <= 0) {
				upY = upY - 50
				downY = downY - 50
				middleY = middleY - 50
			}
			var p1: Laya.Point = PlayerManager.instance._players[opseatid].localToGlobal(new Laya.Point(card.width * scale * 0.5, card.height * scale * 0.5));
			var p2: Laya.Point = this._pokerCon._boxCards.globalToLocal(p1);
			for (var i: number = 0; i < this._topFoldCards.length; i++) {
				card = this._topFoldCards[i];
				card.pos(p2.x, p2.y);
				// if (this._topFoldCards.length > 14) {
				// 	if (i < 14) {
				// 		pX = centerX + dis * (i - half);
			...
			if (!isReconnect && needTween && cards) {
				...
				if (this._topFoldCards.length > 14)
					...
					len = this._topFoldCards.length;
				this.showCardsEffect(cards, centerX, pY, 0.8, noAni);
			}
		}
    }
    ...
    protected layoutRightCards(needTween: boolean, cards: Array<number> = null, isReconnect: boolean = false, noAni: boolean = false): void {
    ...
    protected layoutLeftCards(needTween: boolean, cards: Array<number> = null, isReconnect: boolean = false, noAni: boolean = false): void {
     ...
     * 当鼠标滑过手牌时检测
     */
    protected onCheckMouseOverCards(isOver: boolean): void {
        var card1: number;
        var card2: number;
    ...
    protected shownCards(seatid: number, cards: Array<number>, noAni: boolean = false): void {
        var idx: number = Utils.getDir(seatid);
        var player: PlayerData = BaseGameData.getPlayerDataBySeatid(seatid);

        if (BaseGameData.maxPlayer == 3) {
            if (idx == 2) {
                this.showRightCards(cards, seatid, noAni);
            }
            else if (idx == 4) {
                this.showLeftCards(cards, seatid, noAni);
            }
        }
        else if (BaseGameData.maxPlayer == 2) {
            if (idx == 3) {
                this.showTopCards(cards, seatid, noAni);
            }
        }
        else if (BaseGameData.maxPlayer == 4) {
            if (idx == 3) {
    ...
    protected showRightCards(cards: Array<number>, seatid: number, noAni: boolean = false): void {
        this.removeRightShownCards();
        var card: poker.CardSmallUI;
        for (var i: number = 0; i < cards.length; i++) {
            card = poker.CardSmallUI.borrowCard() as poker.CardSmallUI;
            card.init(false, { scaleX: 1, scaleY: 1, visible: true });
            card.card = cards[i];
            this._rightShowCards.push(card);
            this._pokerCon._boxCards.addChild(card);
        }
        this.layoutShowRightCards(true, cards, noAni);
    }

    protected layoutShowRightCards(needTween: boolean, cards: Array<number> = null, noAni: boolean = false): void {
        if (this._rightShowCards.length) {
            var card: poker.CardSmallUI = this._rightShowCards[0];
            var dis = 24
            var pX: number;
            var pY: number;
            var scale = 0.75;
            var right: number = Math.min(1280, this._pokerCon.width * 0.5 + AppControl.getInstance().stage.width * 0.5) - 183 - card.width * scale * 0.5;
            var upY: number = PlayerManager.instance._players[BaseGameData.nextSeatId].y + card.height * scale * 0.5 - 120;//+20
            var downY: number = PlayerManager.instance._players[BaseGameData.nextSeatId].y + card.height * scale * 0.5 - 76;//+76
            var middleY: number = PlayerManager.instance._players[BaseGameData.nextSeatId].y + card.height * scale * 0.5 - 95;
            // var p1: Laya.Point = PlayerManager.instance._players[BaseGameData.nextSeatId].localToGlobal(new Laya.Point(card.width * scale * 0.5, card.height * scale * 0.5));
            // var p2: Laya.Point = this._pokerCon._boxCards.globalToLocal(p1);
            for (var i: number = 0; i < this._rightShowCards.length; i++) {
                card = this._rightShowCards[i];
                if (i < 10) {
                    pX = right - (10 - 1 - i) * dis;
                    pY = upY;
                }
    ...
    protected removeRightShownCards(): void {
        if (!this._rightShowCards)
            this._rightShowCards = [];
        else {
            while (this._rightShowCards.length) {
                var card: poker.CardUI = this._rightShowCards.pop();
                card.removeSelf();
                poker.CardSmallUI.returnCard(card);
            }
        }
    ...
    protected showLeftCards(cards: Array<number>, seatid: number, noAni: boolean = false): void {
        this.removeLeftShownCards();
        var card: poker.CardSmallUI;
        for (var i: number = 0; i < cards.length; i++) {
            card = poker.CardSmallUI.borrowCard() as poker.CardSmallUI;
            card.init(false, { scaleX: 1, scaleY: 1, visible: true });
            card.card = cards[i];
            this._leftShowCards.push(card);
            this._pokerCon._boxCards.addChild(card);
        }
        this.layoutShowLeftCards(true, cards, noAni);
    }

    protected layoutShowLeftCards(needTween: boolean, cards: Array<number> = null, noAni: boolean = false): void {
        if (this._leftShowCards.length) {
            var card: poker.CardSmallUI = this._leftShowCards[0];
            var dis = 24
            var pX: number;
            var pY: number;
            var scale = 0.75;
            var left: number = Math.max(0, this._pokerCon.width * 0.5 - AppControl.getInstance().stage.width * 0.5) + 183 + card.width * scale * 0.5;
            var upY: number = PlayerManager.instance._players[BaseGameData.preSeatId].y + card.height * scale * 0.5 - 110;
            var downY: number = PlayerManager.instance._players[BaseGameData.preSeatId].y + card.height * scale * 0.5 - 54;
            var middleY: number = PlayerManager.instance._players[BaseGameData.preSeatId].y + card.height * scale * 0.5 - 105;
            var p1: Laya.Point = PlayerManager.instance._players[BaseGameData.preSeatId].localToGlobal(new Laya.Point(card.width * scale * 0.5, card.height * scale * 0.5));
            var p2: Laya.Point = this._pokerCon._boxCards.globalToLocal(p1);
            for (var i: number = 0; i < this._leftShowCards.length; i++) {
                card = this._leftShowCards[i];
                if (i < 10) {
                    pX = left + i * dis;
                    pY = upY;
                }
    ...
    protected removeLeftShownCards(): void {
        if (!this._leftShowCards)
            this._leftShowCards = [];
        else {
            while (this._leftShowCards.length) {
                var card: poker.CardUI = this._leftShowCards.pop();
                card.removeSelf();
                poker.CardSmallUI.returnCard(card);
            }
        }
    ...
    protected showTopCards(cards: Array<number>, seatid: number, noAni: boolean = false): void {
        this.removeTopShownCards();
        var card: poker.CardSmallUI;
        for (var i: number = 0; i < cards.length; i++) {
            card = poker.CardSmallUI.borrowCard() as poker.CardSmallUI;
            card.init(false, { scaleX: 1, scaleY: 1, visible: true });
            card.card = cards[i];
            this._topShowCards.push(card);
            this._pokerCon._boxCards.addChild(card);
        }
        this.layoutShowTopCards(true, cards, noAni);
    }

    protected layoutShowTopCards(needTween: boolean, cards: Array<number> = null, noAni: boolean = false): void {
        if (this._topShowCards.length) {
            var card: poker.CardSmallUI = this._topShowCards[0];
            var dis = 24
            var pX: number;
            var pY: number;
            var scale = 0.4;
            var centerX: number = this._pokerCon.width * 0.5+50;
            let opseatid = (BaseGameData.selfSeatid + 1) % 4 + 1
            var upY: number = PlayerManager.instance._players[opseatid].y + card.height * scale * 0.5 + 20 //- 120;//+20
            var downY: number = PlayerManager.instance._players[opseatid].y + card.height * scale * 0.5 + 64 //- 76;//+76
            var middleY: number = PlayerManager.instance._players[opseatid].y + card.height * scale * 0.5 + 83 //- 95;
            var half: number = Math.ceil(this._topShowCards.length * 0.5) - 1;


            for (var i: number = 0; i < this._topShowCards.length; i++) {
                card = this._topShowCards[i];
                //if (i < 10) {
                pX = centerX + dis * (i - half) + 20
                pY = upY;
                // }
                // else {
                //     pX = centerX + dis * (i - 10 - half);
    ...
    protected removeTopShownCards(): void {
        if (!this._topShowCards)
            this._topShowCards = [];
        else {
            while (this._topShowCards.length) {
                var card: poker.CardUI = this._topShowCards.pop();
                card.removeSelf();
                poker.CardSmallUI.returnCard(card);
            }
        }
    }
    protected showMyFriendCards(cards: Array<number>, seatid: number, noAni: boolean = false): void {
        this.removeMyFriendShownCards();
        var card: poker.CardBigUI;
        for (var i: number = 0; i < cards.length; i++) {
            card = poker.CardBigUI.borrowCard() as poker.CardBigUI;
            card.init(false, { scaleX: 1, scaleY: 1, visible: true });
            card.card = cards[i];
            card.zOrder = i+1
            this._friendShowCards.push(card);
            this._pokerCon._boxCards.addChild(card);
        }
        this.layoutMyFriendCards(true, cards, noAni);

    }
    protected layoutMyFriendCards(needTween: boolean, cards: Array<number> = null, noAni: boolean = false): void {
        if (this._friendShowCards.length) {
            let len
            if (AppControl.getInstance().stage.width < this._table._bg.width) {
                len = AppControl.getInstance().stage.width
            }
            else {
                len = this._table._bg.width
            }
            var dis: number = (len - 100) / this._friendShowCards.length;

            if (dis > 50)
                dis = 50;
            var centerX: number = this._pokerCon.width * 0.5;
            var half: number = Math.ceil(this._friendShowCards.length * 0.5) - 1;
            var pX: number;
            var pY: number;
            var time: number = 0;
            var delay: number = 0;
            for (var i: number = 0; i < this._friendShowCards.length; i++) {
                pX = centerX + dis * (i - half);
                this._friendShowCards[i].selected = true;
                this._friendShowCards[i].move({ x: pX, y: AppControl.getInstance().stage.height - 640 + this._myHandCardPosiY - 27, visible: true, scaleX: 0.8, scaleY: 0.8 }, true, false, needTween, time, null, null, null, delay);
            }
        }
    }

    protected removeMyFriendShownCards(): void {
        if (!this._friendShowCards)
            this._friendShowCards = [];
        else {
            while (this._friendShowCards.length) {
                var card: poker.CardUI = this._friendShowCards.pop();
                card.removeSelf();
                poker.CardBigUI.returnCard(card);
            }
        }
