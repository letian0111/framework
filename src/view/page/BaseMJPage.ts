class BaseMJPage extends MatchPage {
    ...
    protected init(...params): void {
        ...
        BaseFlowerCardManager.instance.registerClass(BaseFlowerCardView)
    }
    ...
    protected initGameView() {
        ...
        if (!this._leftView) {
            ...
            this._leftView.left = AppControl.getInstance().stage.width > TableLayout.STAGE.width ? (AppControl.getInstance().stage.width - TableLayout.STAGE.width) / 2 +10 : 10;
            this._leftView.top = 70
    ...
    protected setTableInfo(): void {
        ...
        if (!BaseGameData.isRecord) {
            ...
            this._tablePop._inviteUI.visible = checkStart
        }
    }
    ...
    protected clearCardsView() {
        ...
        BaseFlowerCardManager.instance.clearAll()
    ...
    protected initCardsView() {
        ...
        for (var k in BaseGameData.players) {
            ...
            let FlowerPos = TableLayout.FlowerCardLayouts[player.dir - 1]
            if (BaseFoldCardManager.instance._foldType == 0) {
            ...
            let flowerView = BaseFlowerCardManager.instance.addFlowerCardView(player.seatid, player.dir)
            if (flowerView) {
                this._table.addChild(flowerView)
                if (player.dir == 1) {
                    if (Laya.stage.width / Laya.stage.height < 8 / 5) {
                        let scale = Laya.stage.width / 1024
                        flowerView.left = Laya.stage.width > 1280 ? (Laya.stage.width - 1280) / 2 + 100 : 100
                    } else {
                        flowerView.centerX = FlowerPos.x
                    }
                    flowerView.bottom = FlowerPos.y
                } else if (player.dir == 2) {
                    flowerView.right = Laya.stage.width > 1280 ? (Laya.stage.width - 1280) / 2 + FlowerPos.x : FlowerPos.x
                    flowerView.y = FlowerPos.y
                } else if (player.dir == 3) {
                    flowerView.centerX = FlowerPos.x
                    flowerView.y = FlowerPos.y
                } else {
                    flowerView.left = Laya.stage.width > 1280 ? (Laya.stage.width - 1280) / 2 + FlowerPos.x : FlowerPos.x
                    flowerView.y = FlowerPos.y
                }
                // let order = v.dir == 4 and FOLD_CARD_ZORDER or HAND_CARD_ZORDER
                // holdView:addTo(self,order):pos(holdCardLayouts[v.dir].x, holdCardLayouts[v.dir].y)
    ...
    protected onResize(e: laya.events.Event): void {
        ...
        if (!this._scale || Laya.stage.width / Laya.stage.height != this._scale) {
            ...
            if (Laya.stage.width / Laya.stage.height < 16 / 9) {
                Laya.stage.scaleMode = Laya.Stage.SCALE_FIXED_WIDTH;
    ...
    protected onGameStartNtfHandler(msg: any) {
        ...
        if (localStorage.getItem("rule_tip_2") == "1") {
    ...
    protected setFlowerCards() {
        for (var k in BaseGameData.players) {
            let player = BaseGameData.players[k]
            if (player.flowerCards && player.flowerCards.length > 0) {
                BaseFlowerCardManager.instance.updateFlowerCards(player.seatid, player.flowerCards)
            }
        }
    }
    ...
    protected onShowInfoNtfHandler(msg: any): void {
        ...
        switch (msg.type) {
            case GameDef.ShowInfoType.SHOW_DICE:
                ...
                break;
            case GameDef.ShowInfoType.LIAN_ZHUANG:
                BaseGameData.saveRemainBanck(msg.info)
            ...
            case GameDef.ShowInfoType.BAO_TING:
                ...
                break;
            case GameDef.ShowInfoType.ZHUA_MA:
                this.playZhuaMa(msg.info, msg.animation);
                break;
        }
    }
    protected playZhuaMa(info, ani){
    ...
    protected showDrawShifter(info) {
        BaseGameData.SHIFTER_NUM = info.info
        if (info.noAni) {
            if (this._leftView) {
        ...
        if (info.animation) {
            BaseGameData.leftCard = BaseGameData.leftCard - BaseGameData.SHIFTER_NUM.length
            // AnimationManager.instance.addAnimation(() => {
    ...
    protected onReconnectNtfHandler(msg: any) {
        ...
        if (this._leftView) {
            ...
            if (BaseGameData.SHIFTER_NUM && BaseGameData.SHIFTER_NUM.length > 0) {
                this._leftView.updateShifter()
            } else {
    ...
    protected askPlayerOpt(seatid, timeout, opt) {
        ...
        if (opt && opt.length > 0) {
            ...
            if (Utils.checkSeatid(BaseGameData.selfSeatid)) {
                ...
                for (var k in opt) {
                    let v = opt[k]
                    if (v.opttype > GameDef.OptType.MJ_DRAW && v.opttype < GameDef.OptType.MJ_MISS_HU || v.opttype == GameDef.OptType.MJ_MUST_HU) {
                        optFlag = true
                    }
                    if (v.opttype == GameDef.OptType.MJ_CHI || v.opttype == GameDef.OptType.MJ_PENG || v.opttype == GameDef.OptType.MJ_MNGANG) {
                        showLast = true
                    }
                    if ((v.opttype == GameDef.OptType.MJ_HU || v.opttype == GameDef.OptType.MJ_MUST_HU) && BaseGameData.optSeatid != BaseGameData.selfSeatid) {
                        showLast = true
                    }
                    ...
                    if ((seatid == BaseGameData.selfSeatid) && (v.opttype == GameDef.OptType.MJ_MUST_HU)) {
                        BaseGameData.mustWin = true
                    }
                }
                if (seatid == BaseGameData.selfSeatid) {
                    ...
                    if (optFlag) {
                        ...
                        if (!this._optChoice) {
                            let model = this.getOptChoiceModel()
                            this._optChoice = new model(this.optCardReq.bind(this))
    ...
    protected showFoldBigCard(dir, card) {
        ...
        this._foldBigCard.centerX = TableLayout.FOLD_BIG_CARD[dir - 1].x
        this._foldBigCard.y = TableLayout.FOLD_BIG_CARD[dir - 1].y
    }
    ...
    protected moveFoldCard(seatid, cardid, optcallback?) {
        if (!BaseGameData.moveCard) {
            ...
            let callback = function () {
                ...
                // if (BaseGameData.moveCard){
                //     BaseGameData.moveCard.removeSelf()showAniTip
                //     BaseGameData.moveCard = null
                // }
    ...
    protected onCardMoveNtfHandler(msg: any): void {
        ...
        if (msg.areaid == GameDef.AREA_TYPE.HAND_CARD) {
            ...
            if (msg.opttype == GameDef.OptType.MJ_DEAL) {
                AnimationManager.instance.addAnimation(() => {
                    BaseHandCardManager.instance.showHandCards(msg.toSeatid, msg.cards, true)
                    AnimationManager.instance.animationOver()
                }, null)
                ...
                AnimationManager.instance.addAnimation(() => {
                    ...
                    if (BaseGameData.gameType == GameDef.GAME_TYPE.FUYANG_MJ && msg.cards[0]%100 == 69) {
                        AnimationManager.instance.animationOver()
                        return
                    }
                    if (BaseGameData.moveCard) {
    ...
    protected flowerCardAni(msg) {
        if (msg.opttype == GameDef.OptType.MJ_FLOWER) {
            ...
            BaseFlowerCardManager.instance.addFlowerCards(msg.toSeatid, msg.cards)
        }
    }
    ...
    protected winCardAni(msg) {
        ...
        if (msg.toSeatid == msg.fromSeatid && BaseGameData.isGameing) {
            let p = BaseGameData.getPlayerDataBySeatid(msg.fromSeatid)
            if (p.handCards && p.handCards.length != 13) {
    ...
    */
    ...
    public setBg(idx: number = -1): void {
        let key = "table_bg" + BaseGameData.gameType
        if (-1 === idx) {
            idx = parseInt(localStorage.getItem(key) || "0");
        }
        let bgSources = GameDef.GAME_BG[BaseGameData.gameType]
        let len = bgSources.length
        idx %= len
        let bg = bgSources[idx]
