...
class Server extends Laya.EventDispatcher {
    ...
    */
    ...
    public playerTableOptReq(type: any):void{
        this.send(EventNames.GAME_PLAYER_TABLE_OPT_REQ, { type });
    }
    /**
     * 请求实时战绩
     */
    ...
    public uploadInfoReq(): void {
        this.send(EventNames.GAME_UPLOAD_INFO_REQ, { addr: server.address, longitude: server.longitude ? server.longitude : -1000, latitude: server.latitude ? server.latitude : -1000 });
    }
    ...
    //玩家请求改变自身某一状态
    public playerTableStatusReq(type: number = 1, status: number): void {
        this.send(EventNames.GAME_PLAYER_TABLE_STATUS_REQ, {type, status});
    }   
