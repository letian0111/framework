module component {
    ...
    export class BasePlayer extends laya.ui.View {
        ...
        protected checkMaster() {
            ...
            if (BaseGameData.gameType == GameDef.GAME_TYPE.TONGLU_MJ || BaseGameData.gameType == GameDef.GAME_TYPE.FUYANG_MJ
                || BaseGameData.gameType == GameDef.GAME_TYPE.HONGZHONG_MJ) {
                let index = BaseGameData.remainBanck || 0
                if (index == 2) {
                    index = 3
                }
               
                for (let k in BaseGameData.ruleInfo) {
                    let rule = BaseGameData.ruleInfo[k]
                    rule.value = rule.value || 0
                    if (rule.name == 4 && rule.value == 0) {
                        index = 0
                    }
                }
                index = BaseGameData.gameType == GameDef.GAME_TYPE.TONGLU_MJ ? index : 0
                this.view._imgMaster.skin = "table/table_icon_zhuang_" + index + ".png"
            }
        }
