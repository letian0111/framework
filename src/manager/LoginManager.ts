    ...
    */
    ...
    private onVerifyResponse(response): void {
        ...
        if (response.code == 0) {
            ...
            GameLogic.userData.setItem('gold', this._loginData.gold);
            GameLogic.userData.setItem('phone', this._loginData.phone);
            GameLogic.userData.setItem('ticket', this._loginData.ticket);
            this.loginSucc();
        }
        else {
            ...
            webService.joinTable(this._roomcode, (response: any) => {
                if (response.code == 0) {
                    ...
                    if (!AppControl.getInstance().currentPage || AppControl.getInstance().currentPage instanceof LoginPage)
                        AppControl.getInstance().showPage(GameConfig.getHallPage(), { room_code: LoginManager.instance._roomcode });
                    else if (response.code == 1207)
