...
class BaseGameData {
    ...
    public static isTableEnd: boolean;//桌子是否已经结束
    ...
    public static gameEndInfo = []
    public static SHIFTER_NUM = []
    ...
    public static remainBanck:number; // 连庄标识
    public static daBanwinType;     //打板胜利标志
    public static selfNickname: string;
    public static mustWin: boolean;   // 必须胡牌 不能打牌
    ...
    public static isTqkc =  false                                       //默认无提前开车
   ...
   */
    ...
    public static cardMove(msg: any): void {
        ...
        else if(info.areaid == GameDef.AREA_TYPE.QI_PAI &&info.opttype != GameDef.OptType.RECONNECT){
            BaseGameData.removeHandCard(info)
            seatinfo.foldCards.push(info.cards[0])
            BaseGameData.discards.push(info.cards[0])
    ...
    private static removeHandCard(info: any): void {
        ...
        {   
            ...
            for (var i: number = 0; i < info.cards.length; i++) {
                ...
                else if(seatInfo.handCards[0] <=0){

                    seatInfo.handCards.splice(0, 1);
                }
    ...
    public static resetPos() {
        for (var k in BaseGameData.players) {
            ...
            player.dir = Utils.getDir(player.seatid)
            //"MSG": "http:\/\/tongjiang.queyoujia.com\/main.min.js?0.6684384964828827[1,151240]\nUncaught TypeError: Cannot set property 'dir' of undefined"
            //2018／5/15 14:00
            if(PlayerManager.instance._players[player.seatid]){
    ...
    public static saveRemainBanck(info) {
        if (!info || info.length <= 0) {
            return
        }        
        BaseGameData.remainBanck = info[0]
        let p = BaseGameData.getPlayerDataBySeatid(this.btnSeatid)
