...
class NativeHelper {
    ...
    public wxPay(callback: Function, data): void {
        if (!Laya.Browser.window.conchConfig) {
            return;
        }
        ...
        if (this.isAndroid) {
            let call = Laya.PlatformClass.createClass("com.vendor.social.Social");
            call.callWithBack(function (json) {
                log("android wxpay callback:" + json);
                let result: any = JSON.parse(json);
                if (result.result == 0) {
                    callback("0");
        ...
        if (this.isIOS) {
            // NSString *str = [NSString stringWithFormat:@"weixin://app/%@/pay/?nonceStr=%@&package=Sign%%3DWXPay&partnerId=%@&prepayId=%@&timeStamp=%@&sign=%@&signType=SHA1",[params valueForKey:@"appid"],[params valueForKey:@"noncestr"],[params valueForKey:@"partnerid"],[params valueForKey:@"prepayid"],[params valueForKey:@"timestamp"],[params valueForKey:@"sign"]];
            //weixin://wap/pay?prepayid%3Dwx24165603543962e34b0eaa9a1410900038&package=4248067039&noncestr=1535100995&sign=7473d2f7d7d211c661a03b223e2b4eca
            let url = "weixin://app/" + GameConfig.WX_APPID + "/pay/?nonceStr=" + data.nonceStr + "&package=Sign%3DWXPay&partnerId=" + data.partnerId + "&prepayId=" + data.prepayId + "&timeStamp=" + data.timeStamp + "&sign=" + data.sign + "&signType=SHA1"

            let call = Laya.PlatformClass.createClass("NativeHelper");
            call.callWithBack(function (json) {
                log("ios wxpay callback:" + json);
                let result: any = JSON.parse(json);
                if (result.result == 0) {
                    callback("0");
                } else {
    ...
    public getWXisInstall(): boolean {
        console.log("getWXisInstall================")
        if (this.isIOS) {
            let call = Laya.PlatformClass.createClass("NativeHelper");
            call.callWithBack(function (json) {
                log("getWXisInstall callback:" + json);
                let result: any = JSON.parse(json);

                if (result.isInstall == 1) {
    ...
    public getLocation(): void {
        ...
        if (this.isIOS) {
            let call = Laya.PlatformClass.createClass("NativeHelper");
            call.callWithBack(function (json) {
                log("getLocation callback:" + json);
                let result: any = JSON.parse(json);


                server.latitude = result.la;
                server.longitude = result.long;
    ...
    public getBundleID(callback: Function): string {
        ...
        if (this.isIOS) {
            ...
            let call = Laya.PlatformClass.createClass("com.vendor.NativeHelper");
            let bundleId = call.call("getBundleID");
            if (callback) {
                callback(bundleId)
            }
        }
    ...
    public openWebView(data): void {
        if (this.isIOS) {
            let call = Laya.PlatformClass.createClass("NativeHelper");
            log("openWebView:"+ data.mweb_url)
            call.call("openWebView:", data.mweb_url);
        
        } else if (this.isAndroid) {
            let call = Laya.PlatformClass.createClass("com.vendor.NativeHelper");
            call.call("openWebView", data.payInfo);
        }
    }

    public towxpay(url: string): void {
        if (this.isAndroid) {
     ...
     * @param {string} url 文件网络URL地址
     * @param {Function} callback 
     * @returns {string} 
     * @memberof NativeHelper
     */
    public downloadFile(path: string, url: string, callback: Function) {
        if (!Laya.Browser.window.conchConfig) {
            return;
        }
        if (this.isIOS) {
            let call = Laya.PlatformClass.createClass("NativeHelper");
            call.callWithBack(function (filePath) {
                if (callback) {
                    callback(filePath)
                }
            }, "downloadFile:withUrl:", path, url);
        } else if (this.isAndroid) {
        }
    }
}