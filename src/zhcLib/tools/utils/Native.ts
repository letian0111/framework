...
class Native extends Laya.EventDispatcher {
   ...
   */
	...
	public getMatchRankInfo(info, type, subTitle, des) {
		var link: string = GameConfig.GAME_URL + "agent/loadShare/index.html";
		var desc: string = "";
		var title: string = "";
		...
		title += StringUtils.format(GameConfig.language.share_3, createInfo.ch_name + text) + subTitle;
		desc += des;
		...
		if (Native.instance.isNative) {
			...
			if (type == 1) {
				...
				NativeHelper.instance.wxShareWebPage(pos, data.link, "", data.title, data.desc, function (result) {
					console.info(result)
					console.log("pos=============" + pos)
					if (result.result == 0 && pos == 1) {
						webService.shareDiamond(function (data) {
							// if (data.code == 0){
							GameLogic.selfData.getInfo(true);
							localStorage.setItem("share_" + server.uid, String(new Date().getTime()))
							Dispatcher.dispatch("hideshare")
							ShareDailyCtrl.instance.hide()
							HintCtrl.instance.show(data.message);
						})

						webService.lotteryChance()
					}
				});
				...
				xhr.once(Laya.Event.COMPLETE, this, this.completeHandler, [pos]);
				xhr.once(Laya.Event.ERROR, this, this.errorHandler);
				xhr.send(info, "", "get", "arraybuffer");
			}
			//分享截屏
			else {
				Laya.Browser.window.conch.captureScreen(function (arrayBuff, width, height) {
					...
					//存储文件的方式
					Laya.Browser.window.conch.saveAsPng(arrayBuff, width, height, Laya.Browser.window.conch.getCachePath() + "/share_screen.png");
					Laya.Browser.window.globalImage = window.document.createElement("img");
					Laya.Browser.window.globalImage.onload = function () {
						// ...使用image对象

					}
					Laya.Browser.window.globalImage.src = "file:///" + Laya.Browser.window.conch.getCachePath() + "/share_screen.png";
					log("[SHARE] file:" + Laya.Browser.window.conch.getCachePath() + "/share_screen.png");

					NativeHelper.instance.wxShareImg(pos, Laya.Browser.window.conch.getCachePath() + "/share_screen.png", function (result) {
	...
	public completeHandler(pos, data) {
		Laya.Browser.window.fs_writeFileSync(Laya.Browser.window.conch.getCachePath() + "/share.png", data);
	...
	public getShareInfo(): any {
		...
		var imgUrl: string = GameConfig.WX_URL + "shareIcon/" + GameConfig.APPAREA + ".png";
		if (server.code) {
			link += "?page=1&cmd=sharegame&gamearea=" + GameConfig.APPAREA + "&roomcode=" + server.code + "&game_rule=" + GameConfig.DESC[server.code] + "&gametype=" + BaseGameData.gameType+"&sub_id="+GameConfig.SUB_ID;
			...
			if (matchRule) {
				...
				title += StringUtils.format(GameConfig.language.share_2, createInfo.ch_name + text, server.code);
				link += "&match_rule=" + matchRule;

			}
			else {
				title += StringUtils.format(GameConfig.language.share_2, createInfo.ch_name, server.code);
				if (tmp)
					desc += tmp + ",就等你来嗨！";
			}
			link += "&pid=" + server.uid;
			...
			link += "?page=2&cmd=sharegame&gamearea=" + GameConfig.APPAREA+"&sub_id="+GameConfig.SUB_ID;
			link += "&pid=" + server.uid;

			if (GameConfig.APPAREA == "juezhan") {
				title += StringUtils.format(GameConfig.language.share_juezhan, GameConfig.APPNAME);
			}
			else if (GameConfig.APPAREA == "tongjiang") {
				title += StringUtils.format(GameConfig.language.share_1, GameConfig.APPNAME);
			}
			else if (GameConfig.APPAREA == "zhangxin") {
				title += StringUtils.format(GameConfig.language.share_zhangxin, GameConfig.APPNAME);
			}
	...
	public iapRecharge(id: number, pkey: string): void {
		LoadingUI.instance.show(LoadingUI.TYPE_NO_BG_ATONCE, null, null, null, 30000);
		webService.rechagre(id, "", (response) => {
			if (response.code != 0 || response.data == null || response.data.product_id == null || response.data.order_id == null) {
				...
				let str = JSON.stringify({ product_id: pkey, order_id: response.data.order_id })
				let call = Laya.PlatformClass.createClass("IAPManager");
	 ...
	 * @param id 商品id
	 */
	public recharge(id: number, pkey: string): void {
		//测试
		//this.iapRecharge(id);
		...
		if (Native.instance.isNative) {
			...
			if(Native.instance.isIOS&&GameConfig.CHANNEL && (GameConfig.APP_VER=="1.0.0"||GameConfig.APP_VER=="1.00"||GameConfig.APP_VER=="2.00"||GameConfig.APP_VER=="2.0.0")) {
				isInHouse = true
			}
			...
			if (this.OS == OSConfig.IOS_NATIVE_STORE && !isInHouse) {
				...
				webService.rechagre(id, "", (response) => {
					if (response.code == 0) {
						...
						if(GameConfig.CHANNEL == "juezhan"||GameConfig.CHANNEL == "badao"||GameConfig.CHANNEL == "zhangxin"){
							NativeHelper.instance.openWebView(response.data)
						}else{
							NativeHelper.instance.wxPay(function (code: string) {
								if (code.length > 0) {
			...
			//不在微信浏览器
				...
				webService.rechagre(id, cburl, (response) => {
					if (response.code == 0) {
						// Native.instance.showIframe(response.data.payInfo);
						localStorage.setItem(GameLogic.selfData.uid + GameConfig.CP_TRAN_NO, response.data.order_id);
						Laya.Browser.window.location.href = response.data.payInfo;
	...
	private shopOrderFail(code, msg = ""): void {
	...
	*/
	...
	private onEnterBackGround(isEnterBackGround): void {
		if (isEnterBackGround) {
			...
			if(GameConfig.CHANNEL =="honghu"){
				SoundManager.instance.playBg("bg_honghu");
			}
			if(GameConfig.CHANNEL =="zhangxin"){
				SoundManager.instance.playBg("bg_zhangxin");
			}
			else
			{
	...
	*/
	...
	public gotoDownload(): void {
		var url: string = GameConfig.GAME_URL + "agent/loadShare/index.html?gamearea=" + GameConfig.APPAREA+"&sub_id="+GameConfig.SUB_ID;
